...
from run_prompt import execute_gemini_for_tweet_comparison
...
def create_tweet(prompt):
    with open("analyzed_tweets.json") as f:
        ...
        system_prompt = f"""
                            Create a engaging twitter tweet for my company according to the given prompt
                            PROMPT: {prompt}
        ...
        prompt_for_comparing = f"""
            ...
            Tweet A: {tweet_a}
            Tweet B: {tweet_b}
            Your task is to compare both tweets with the sample set of best performing tweets found
        ...
        prediction_res = execute_gemini_for_tweet_comparison(
            prompt=prompt_for_comparing,
            model="gemini-2.5-flash-lite"
        )
        prediction_dict=json.loads(prediction_res)
        tweet_a_vs_tweet_b=prediction_dict['tweet_a_vs_tweet_b']
